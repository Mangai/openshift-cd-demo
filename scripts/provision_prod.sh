...
GITHUB_ACCOUNT=${GITHUB_ACCOUNT:-Mangai}
...
function deploy() {
  ...
  oc $ARG_OC_OPS new-project prod-$PRJ_SUFFIX --dispaly-name="Tasks - Prod"
  ...
  if [ $LOGGEDIN_USER == 'system:admin' ] ; then
    ...
    oc $ARS_OC_OPS adm policy add-role-to-user admin $ARG_USERNAME -n prod-$PRJ_SUFFIX >/dev/null 2>&1
    ...
    oc $ARG_OC_OPS annotate --overwrite namespace prod-$PRI_SUFFIX demo=openshift-cd-$PRJ_SUFFIX >/dev/null 2>&1
  ...
  local template=https://raw.githubusercontent.com/$GITHUB_ACCOUNT/openshift-cd-demo/blob/$GITHUB_REF/cicd-template_modify.yaml
